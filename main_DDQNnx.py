...
BATCH_SIZE = 1      # batch size for each training 
MAX_MEM = 2         # size of memory in experience replay
PROB = 0.5          # probability of skittle generation
EPISODES = 5000     # total episodes 
REPLACE = 20        # number of episodes after which target is replaced by eval 
...
PATH = Path("""/Users/ritwik/Desktop/gym-factory/DDQN_no_ex/""")
...
class Agent(object):
    ...
    # function to store (state, action, speed, reward, new_state)
    ...
    def learn(self, batch_size):
        ...
        Qnext = self.Q_next.forward(list(memory[:, 4])).to(self.Q_eval.device) # feedforward the final states 
...
def main():
    ...
    brain = Agent(gamma = 0.9,alpha=0.0001, maxMemorySize=MAX_MEM,replace=REPLACE)
